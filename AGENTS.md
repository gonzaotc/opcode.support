...
| `chains.json` | pinned top 30 with health-checked RPC pool | never by hand, run `npm run chains` |
| `extra-rpcs.json` | supplementary endpoints per chain id | a chain has fewer than 2 working RPCs |
...
4. A status is recorded only when **two calibrated endpoints agree**.
...
- **Two witnesses.** One endpoint is one witness and can misreport.
...
Re-run `npm run probe` and diff verdicts against the previous `data/results.json`. Verdicts should
be stable run to run; anything that moves is either a real chain change or a bug in your change.
